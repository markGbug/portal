package org.hango.cloud.ncegdashboard.apiserver.web.controller;
...
public class SwaggerControllerTest extends BaseServiceImplTest {
	...
	public static String version = "2019-09-01";
	...
	public void init() {
		mockMvc = MockMvcBuilders.webAppContextSetup(context).addFilters(new RequestContextHolderFilter()).
			                                                                                                  addFilters(
				                                                                                                  new LogUUIDFilter())
		                         .addFilters(new ProjectTraceFilter()).build();
		//初始化ServiceInfo
	...
	public void getServicesDocs() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get(Const.ENVOY_GATEWAY_PREFIX).param("Version", version).param("Action",
		                                                                                   "DescribeSwaggerServiceById")
		                                      .param("ServiceId", String.valueOf(serviceId)).header("x-auth-projectId",
		                                                                                            projectId).header(
				"x-auth-tenantId", tenantId).header("x-auth-accountId", accountId).header("x-auth-token", tenantId))
	...
	public void getInterfacesDocs() throws Exception {
		mockMvc.perform(MockMvcRequestBuilders.get(Const.ENVOY_GATEWAY_PREFIX).param("Version", version).param("Action",
		                                      ...
		                                      .param("InterfaceId", String.valueOf(apiId)).header("x-auth-projectId",
		                                                                                          projectId).header(
				"x-auth-tenantId", tenantId).header("x-auth-accountId", accountId).header("x-auth-token", tenantId))
		       .andExpect(MockMvcResultMatchers.status().isOk()).andReturn();
	}
