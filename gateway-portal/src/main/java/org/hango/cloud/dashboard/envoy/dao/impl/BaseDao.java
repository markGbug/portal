...
public abstract class BaseDao {
	 ...
	 * 模糊查询前，值需要先调用该方法进行转移
	 * <p>
	 * 在MySQL中，下划线（_）、百分号（%）是一个特殊字符，分别代表一个字符和任意多个字符
	 * 所以在模糊查询使用like前将值中的下划线（_）、百分号（%）进行转移
	 *
	 * @param paramValue 待转义的字符串
	 *
	 * @return 转义后的字符串
	 */
	public static String escapeValueBeforeLike(String paramValue) {
		paramValue = paramValue.replaceAll("_", "\\\\_");
