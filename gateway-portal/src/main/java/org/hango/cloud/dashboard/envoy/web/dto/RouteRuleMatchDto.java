...
public class RouteRuleMatchDto {
	 ...
	 */
	@JSONField(name = "Uri")
	@Valid EnvoyRouteStringMatchDto uriMatchDto;

	/**
	 * 路由规则，methodMatchDto
	 */
	@JSONField(name = "Method")
	@Valid EnvoyRouteStringMatchDto methodMatchDto;

	/**
	 * 路由规则，hostMatchDto
	 */
	@JSONField(name = "Host")
	@Valid EnvoyRouteStringMatchDto hostMatchDto;

	/**
	 * 路由规则，headers
	 */
	@JSONField(name = "Headers")
	@Valid List<EnvoyRouteRuleMapMatchDto> headers;

	/**
	 * 路由规则，queryParams
	 */
	@JSONField(name = "QueryParams")
	@Valid List<EnvoyRouteRuleMapMatchDto> queryParams;
	...
	public void toRouteMeta(RouteRuleMatchInfo sourceMatchInfo) {
		...
		if (CollectionUtils.isNotEmpty(this.headers)) {
			...
			sourceMatchInfo.setHeaderList(
				headers.stream().map(EnvoyRouteRuleMapMatchDto::toMeta).collect(Collectors.toList()));
		...
		if (CollectionUtils.isNotEmpty(this.queryParams)) {
			//queryParam值按照字典序排序
			this.queryParams = queryParams.stream().map(EnvoyRouteRuleMapMatchDto::sortValue).collect(
				Collectors.toList());
			sourceMatchInfo.setQueryParamList(
				queryParams.stream().map(EnvoyRouteRuleMapMatchDto::toMeta).collect(Collectors.toList()));
