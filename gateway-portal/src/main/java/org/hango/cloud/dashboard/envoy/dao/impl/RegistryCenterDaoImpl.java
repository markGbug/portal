...
import org.hango.cloud.ncegdashboard.envoy.dao.IRegistryCenterDao;
...
public class RegistryCenterDaoImpl extends BaseDao implements IRegistryCenterDao {
	...
	public long add(RegistryCenterInfo registryCenterInfo) {
		...
		String sql =
			" insert into apigw_gportal_registry_center (registry_type, registry_addr,registry_alias, create_date, "
			+ "modify_date, project_id)"
		...
		logger.info("addRegistryCenterInfo {}",
		            ReflectionToStringBuilder.toString(registryCenterInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public long update(RegistryCenterInfo registryCenterInfo) {
		String sql =
			" update apigw_gportal_registry_center set registry_type = :registryType, registry_addr = :registryAddr,"
			+ " registry_alias = :registryAlias, modify_date = :modifyDate, project_id= :projectId where id = :id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(registryCenterInfo);
		logger.info("updateRegistryCenterInfo {}",
		            ReflectionToStringBuilder.toString(registryCenterInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public int delete(RegistryCenterInfo registryCenterInfo) {
		...
		logger.info("deleteRegistryCenterInfo {}",
		            ReflectionToStringBuilder.toString(registryCenterInfo, ToStringStyle.SIMPLE_STYLE));
		return 0;
	}
