 ...
 * 2019-11-13
 */
public class EnvoyPluginBindingDto implements Comparable<EnvoyPluginBindingDto> {
	 ...
	 */
	@Pattern(regexp = "routeRule|service|global", message = "插件范围仅支持routeRule/service/global")
	@NotNull
	 ...
	 */
	@Override
	public int compareTo(EnvoyPluginBindingDto o) {
		if (null == o) {
			throw new NullPointerException("the specified object is null");
		}
		// 比较的两个对象不存在equals的情况

		if (this.getPluginPriority() > o.getPluginPriority()) {
			return -1;
		}
		if (this.getPluginPriority() < o.getPluginPriority()) {
			return 1;
	...
	public static EnvoyPluginBindingDto fromMeta(EnvoyPluginBindingInfo bindingInfo) {
		...
		bindingDto.setPluginName(bindingInfo.getPluginType());
