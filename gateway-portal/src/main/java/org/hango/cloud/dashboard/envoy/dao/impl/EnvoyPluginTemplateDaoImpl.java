...
public class EnvoyPluginTemplateDaoImpl extends BaseDao implements IEnvoyPluginTemplateDao {
	...
	public long add(EnvoyPluginTemplateInfo envoyPluginTemplateInfo) {
		...
		String sql =
			"insert into apigw_envoy_plugin_template (create_time, update_time, plugin_type, plugin_configuration, "
			+ "project_id, template_version, template_name, template_notes) "
			+ " values (:createTime, :updateTime, :pluginType, :pluginConfiguration, :projectId, :templateVersion, "
			+ ":templateName, :templateNotes)";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyPluginTemplateInfo);
		namedParameterJdbcTemplate.update(sql, ps, keyHolder);
		logger.info("add envoyPluginTemplateInfo: {}",
		            ReflectionToStringBuilder.toString(envoyPluginTemplateInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public long update(EnvoyPluginTemplateInfo envoyPluginTemplateInfo) {
		String sql = "update apigw_envoy_plugin_template set update_time=:updateTime, "
		             ...
		             + "template_name=:templateName, template_notes=:templateNotes where id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyPluginTemplateInfo);
		logger.info("update envoyPluginTemplateInfo: {}",
		            ReflectionToStringBuilder.toString(envoyPluginTemplateInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public int delete(EnvoyPluginTemplateInfo envoyPluginTemplateInfo) {
		...
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyPluginTemplateInfo);
		logger.info("delete envoyPluginTemplateInfo: {}",
		            ReflectionToStringBuilder.toString(envoyPluginTemplateInfo, ToStringStyle.SIMPLE_STYLE));
		return namedParameterJdbcTemplate.update(sql, ps);
	}
