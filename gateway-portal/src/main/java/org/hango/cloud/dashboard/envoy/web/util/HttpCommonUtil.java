...
public class HttpCommonUtil {
	...
	public static HttpClientResponse getFromApiPlane(String apiPlaneUrl, Map<String, String> params, String body,
		...
		try {
			httpClientResponse = HttpClientUtil.httpRequest(methodType, apiPlaneUrl + "?" + paramsBuilder.substring(1),
			                                                body, headers);
		} catch (Exception e) {
	...
	public static HttpClientResponse getFromAuth(String authUrl, Map<String, String> params, String body,
	                                             Map<String, String> headers, String methodType) {
		headers = headers == null ? new HashMap<>(Const.DEFAULT_MAP_SIZE) : headers;
		headers.put("Content-Type", HttpClientUtil.DEFAULT_CONTENT_TYPE);
		StringBuilder paramsBuilder = new StringBuilder();
		...
		HttpClientResponse httpClientResponse;
		try {
			httpClientResponse = HttpClientUtil.httpRequest(methodType, authUrl + "?" + paramsBuilder.substring(1),
			                                                body, headers);
		} catch (Exception e) {
