 ...
 * 2019-09-11
 */
public class EnvoyRouteRuleDto extends RouteRuleMatchDto {
	...
	@Pattern(regexp = "([\\s\\S]){1,254}", message = "参数 RouteRuleName 不能为空且长度不能超过255")
	...
	public boolean equals(Object obj) {
		...
		EnvoyRouteRuleDto objDto = (EnvoyRouteRuleDto) obj;
		//uri全部相同进行比较
		...
		if (CollectionUtils.isNotEmpty(this.headers) && (!this.headers.containsAll(objDto.getHeaders()) || !objDto
			                                                                                                    .getHeaders()
			                                                                                                    .containsAll(
				                                                                                                    this.headers))) {
			return false;
		}
		if (CollectionUtils.isNotEmpty(this.queryParams) && (!this.queryParams.containsAll(objDto.getQueryParams())
		                                                     || !objDto.getQueryParams().containsAll(
			this.queryParams))) {
			return false;
		}
		return true;
	...
	public EnvoyRouteRuleInfo toMeta() {
