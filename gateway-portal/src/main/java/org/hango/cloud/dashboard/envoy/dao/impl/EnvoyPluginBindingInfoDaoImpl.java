...
public class EnvoyPluginBindingInfoDaoImpl extends BaseDao implements IEnvoyPluginBindingInfoDao {
	...
	public long add(EnvoyPluginBindingInfo envoyPluginBindingInfo) {
		...
		String sql = "insert into apigw_envoy_plugin_binding (plugin_type, binding_object_id, binding_object_type, "
		             + "plugin_configuration, create_time, update_time, gw_id, project_id, plugin_priority, "
		             + "binding_status, " + "template_id, template_version)"
		             + " values (:pluginType, :bindingObjectId, :bindingObjectType, :pluginConfiguration, "
		             + ":createTime, "
		             + ":updateTime, :gwId, :projectId, :pluginPriority, :bindingStatus, :templateId, "
		             + ":templateVersion)";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyPluginBindingInfo);
		namedParameterJdbcTemplate.update(sql, ps, keyHolder);
		logger.info("add envoyPluginBindingInfo: {}",
		            ReflectionToStringBuilder.toString(envoyPluginBindingInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public long update(EnvoyPluginBindingInfo envoyPluginBindingInfo) {
		String sql = "update apigw_envoy_plugin_binding set plugin_configuration=:pluginConfiguration, "
		             ...
		             + "template_version=:templateVersion where id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyPluginBindingInfo);
		logger.info("update envoyPluginBindingInfo: {}",
		            ReflectionToStringBuilder.toString(envoyPluginBindingInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public int delete(EnvoyPluginBindingInfo envoyPluginBindingInfo) {
		...
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyPluginBindingInfo);
		logger.info("delete envoyPluginBindingInfo: {}",
		            ReflectionToStringBuilder.toString(envoyPluginBindingInfo, ToStringStyle.SIMPLE_STYLE));
		return namedParameterJdbcTemplate.update(sql, ps);
	}
	...
	@Override
	public long getBindingPluginCount(long projectId, long gwId, List<Long> gwIdList, String bindingObjectId,
	                                  List<String> bindingObjectIdList, List<String> bindingObjectTypeList,
	                                  String pattern) {
		Map<String, Object> params = new HashMap<>(Const.DEFAULT_MAP_SIZE);
		String sql = "select count(*) from apigw_envoy_plugin_binding where project_id=:projectId";
		params.put("projectId", projectId);

		sql = buildPatternSql(gwId, gwIdList, bindingObjectId, bindingObjectIdList, bindingObjectTypeList, pattern,
		                      params, sql);
	...
	@Override
	public List<EnvoyPluginBindingInfo> getBindingPluginList(long projectId, long gwId, List<Long> gwIdList,
	                                                         String bindingObjectId, List<String> bindingObjectIdList,
	...
	public long batchDeleteBindingInfo(List<EnvoyPluginBindingInfo> bindingInfoList) {
		...
		List<Long> bindingInfoIdList = bindingInfoList.stream().map(EnvoyPluginBindingInfo::getId).collect(
			Collectors.toList());
	...
	public long batchDissociateTemplate(List<Long> bindingInfoIdList) {
		String sql
			= "update apigw_envoy_plugin_binding set template_id = 0 and template_version = 0 where id in (:idList)";
		Map<String, Object> params = new HashMap<>(Const.DEFAULT_MAP_SIZE);
		params.put("idList", bindingInfoIdList);
	...
	private String buildPatternSql(long gwId, List<Long> gwIdList, String bindingObjectId,
	                               List<String> bindingObjectIdList, List<String> bindingObjectTypeList,
	                               String pattern,
		...
		if (StringUtils.isNotBlank(pattern)) {
			if (!CollectionUtils.isEmpty(gwIdList) && !CollectionUtils.isEmpty(bindingObjectIdList)) {
				sql = sql
				      + " and (gw_id in (:gwIdList) or binding_object_id in (:bindingObjectIdList) or plugin_type like"
				      + " :pattern)";
