...
public enum ErrorCodeEnum {
	...
	MissingParameter("MissingParameter",
	                 "The required input parameter %s for processing this request is not " + "supplied.", "参数  %s 缺失",
	                 400),
	...
	SignatureDoesNotMatch("SignatureDoesNotMatch",
	                      "The request signature we calculated does not match the signature you provided.", "签名不匹配",
	                      403),
	InvalidAuthorizationInfo("InvalidAuthorizationInfo", "The authorization info you provided is invalid.", "认证信息无效",
	                         400),
	...
	DryRunOperation("DryRunOperation", "The quest would have successed, but the DryRun parameter was used.",
	                "签名认证通过，但是使用了 DryRun 参数", 400),
	...
	// 用于Namespace
	NameExistAndDeleting("NameExistAndDeleting", "Name %s already exist and is deleting, please retry later.",
	                     "名称%s 已存在且正在删除中，请稍后再试", 400),
	...
	ContainStatusError("InvalidParameterValue", "Container %s status %s not support this operation.",
	                   "容器%s 的状态%s 不允许执行当前操作", 400),
	WorkloadStatusError("InvalidParameterValue", "Workload %s status %s not support this operation.",
	                    "负载%s 的状态%s 不允许执行当前操作", 400),
	NoPermissionBindWorkload("NoPermission", "Bare Container is not allowed to bind service", "高性能容器不允许绑定服务", 403),
	NotMatchBounds("NotMatchBounds", "%s is not  match the bounds", "%s不符合限制", 400),

	// StatefulWorkload
	InvalidWorkloadState("InvalidWorkloadState", "The workload can't operate in current state.", "工作负载处于无法进行此操作的状态。",
	                     400),
	...
	LogDirsAndMountPathDuplicateError("InvalidParameterValue", "Value for %s and for %s are Duplicated",
	                                  "%s 和 %s " + "值有重复", 400),
	CannotUpdateSpec("CannotUpdateSpec",
	                 "The current spec type of this workload is same as the spec type %s you provided, cannot update.",
	                 "工作负载当前规格与传入的规格 %s 一致，无法镜像更新规格操作。", 400),
	CannotRebuild("CannotRebuild",
	              "The charge type of this workload is not 'Package' or package is unchanged, cannot do rebuild "
	              + "operation.", "工作负载的计费方式不为资源包计费或者资源包未表更，无法进行资源包重建操作", 400),
	...
	ChargeTypeIllegal("ChargeTypeIllegal", "The charge type of this workload is %s, can not do this operation.",
	                  "工作负载当前计费类型为 %s，不能进行该操作。", 400),
	NoPermission("NoPermission", "You don't have permission to access the specified interface.", "对不起，您没有权限访问该接口。",
	             401),
	...
	// 计费有关
	InsufficientFunds("InsufficientFunds", "Not sufficient funds in account", "账号余额不足", 403),
	OrderAlreadyUsed("OrderAlreadyUsed", "The order id is already used", "该订单号已经被使用", 403),
	ChargeException("ChargeException", "Charge system exception, please try again or later", "计费异常，请重试", 500),
	...
	TimeRangeTooLarge("TimeRangeTooLarge", "Does not support interval queries greater than %s days.",
	                  "暂不支持大于%s" + "天的区间查询.", 400),
	...
	ServiceNotPublished("ServiceNotPublished", "We can't found the service publish information.", "您指定的Service未发布!",
	                    400),
	ServiceNotAllowUpdateInstance("ServiceNotAllowUpdateInstance",
	                              "you can't update the service instance when choose registry center strategy.",
	                              "注册中心拉取服务方式不能更新实例!", 400),
	CannotPublishApi("CannotPublishApi", "You can't pushlish an interface before publishing a service",
	                 "在未发布服务之前不能发布接口", 400),
	CannotOfflineService("CannotOfflineService", "You can't offline a service before offline all api of this service.",
	                     "在未下线该服务所有接口之前，不允许下线服务", 400),
	...
	InvalidModulusShuntInstanceList("InvalidModulusShuntInstanceList",
	                                "When modulus shunt chosen,the instance list can be only exist one element ",
	                                "取模阈值方式时，实例列表只能存在一条数据", 400),
	...
	ShuntWayBindingNotAvailable("ShuntWayBindingNotAvailable",
	                            "You can only bind shunt way strategy for target type is instance name when service "
	                            + "published by customInput kind", "以手动填写方式发布的服务只能启用目标类型为实例名称的分流策略", 400),
	...
	SameNamePolicyExist("SameNamePolicyExist", "The policy with the same name already exists and cannot be created.",
	                    "同名策略已存在，不允许重复创建!", 400),
	...
	CannotDeleteGateway("CannotDeleteGateway", "Cannot delete gateway,please offline service or api", "网关存在已发布信息，无法删除",
	                    400),
	...
	//服务相关
	CannotUpdateService("CannotUpdateService", "You can't update service until offline the service",
	                    "在下线服务之前，不能修改服务的基本信息", 400),
	ServicePublishedUpdateLimit("ServicePublishedUpdateLimit", "You can't update service publish type",
	                            "不能更新发布服务的发布方式",
	                            400),
	CannotUpdateServiceName("CannotUpdateServiceName", "You can't update service name cause of sync",
	                        "从元数据同步服务，不允许修改服务名称", 400),
	CannotDeleteOnlineService("CannotDeleteOnlineService", "You can't delete service until offline the service",
	                          "在下线服务之前，不能删除服务", 400),
	CannotDeleteApiService("CannotDeleteApiService", "You can't delete service until delete all apis",
	                       "在删除api" + "前，不能删除服务", 400),
	CannotDownloadServiceSDK("CannotDownloadServiceSDK", "You can't download sdk cause of no APIs",
	                         "服务下不存在API，不允许下载SDK", 400),
	InvalidCustomHeaderKey("InvalidCustomHeaderKey", "You can't set custom header key with _ character",
	                       "自定义Header中Key不能使用 _ ", 400),

	//API相关
	AlreadyExistAliasName("AlreadyExistAliasName", "The alias name already exist", "接口标识已经存在", 400),
	AlreadyExistRuleName("AlreadyExistRuleName", "The rule name already exist in current service", "该服务下分流策略名称已经存在",
	                     400),
	CannotDeleteOnlineApi("CannotDeleteOnlineApi", "You can't delete api until offline the api", "在下线接口之前，不能删除接口",
	                      400),
	...
	AlreadyBindTrafficControl("AlreadyBindTrafficControl", "The api already bind traffic control", "API绑定了流控策略，请先解绑",
	                          400),
	...
	InvalidPluginOperation("InvalidPluginOperation", "Plugin couldn't be removed when it was alive", "插件在启用状态下不能将其删除",
	                       400),
	...
	SameNameGatewayExist("SameNameGatewayExist",
	                     "The gateway with the same name already exists and cannot be " + "created.",
	                     "同名网关已存在，不允许重复创建!", 400),
	SameNameGatewayClusterExist("SameNameGatewayClusterExist",
	                            "The gateway cluster with the same name already exists and cannot be created.",
	                            "同名网关集群已存在，不允许重复创建!", 400),
	SameNameRouteRuleExist("SameNameRouteRuleExist",
	                       "The route rule with the same name already exists and cannot be created.",
	                       "同名路由规则已存在，不允许重复创建!", 400),
	SameParamRouteRuleExist("SameParamRouteRuleExist",
	                        "The same param of route rule already exists and cannot be created.",
	                        "相同参数的路由规则已存在，不允许重复创建!", 400),
	NoSuchRouteRule("NoSuchRouteRule", "No such route rule", "指定的路由规则不存在", 400),
	InvalidDestinationService("InvalidDestinationService", "Destination service is invalid", "路由规则发布时指定的后端服务不能为空",
	                          400),
	ServiceAlreadyPublished("ServiceAlreadyPublished", "The service has already published", "服务已经发布到当前网关", 400),
	RouteRuleAlreadyPublished("RouteRuleAlreadyPublished", "The route rule has already published", "路由规则已经发布至该网关",
	                          400),
	RouteRuleAlreadyPublishedToGw("RouteRuleAlreadyPublishedToGw",
	                              "The route rule has already published to gw, gw is: %s.", "路由规则已发布至网关: %s", 400),
	...
	CannotDuplicateBinding("CannotDuplicateBinding",
	                       "The plugin binding already exists and duplicate binding are not allowed",
	                       "插件绑定关系已存在，不允许重复绑定同一插件", 400),
	//健康检查相关
	...
	RouteRuleContainsNginxCapture("RouteRuleContainsNginxCapture", "Route rule contains nginx capture regex",
	                              "创建路由，path正则中不能包含nginx捕获正则", 400),
	...
	BatchPublishRouteError("BatchPublishRouteError", "Batch publish route error, the error gw is: %s.",
	                       "批量发布路由至多网关失败，失败网关: %s", 500),
	ProjectNotAssociatedGateway("ProjectNotAssociatedGateway",
	                            "The current project is not associated with the specified gateway.",
	                            "当前项目未关联指定网关，不允许发布", 400),

	//负载均衡相关
	InvalidLoadBanlanceType("InvalidLoadBanlanceType", "This load balance type is invalid",
	                        "服务负载均衡类型取值为Simple或ConsistentHash", 400),
	InvalidSimpleLoadBanlanceType("InvalidSimpleLoadBanlanceType", "This simple load balance type is invalid",
	                              "Simple类型的负载均衡规则，仅包含ROUND_ROUBIN、LEAST_CONN、RANDOM", 400),
	InvalidConsistentHashObject("InvalidConsistentHashObject", "Consistent hash object is invalid", "一致性哈希对象格式非法",
	                            400),
	InvalidConsistentHashType("InvalidConsistentHashType", "Consistent hash type is invalid",
	                          "一致性哈希对象类型为HttpHeaderName、HttpCookie、UseSourceIp三者之一", 400),
	InvalidConsistentHashHttpCookieObject("InvalidConsistentHashHttpCookieObject", "Http cookie is invalid",
	                                      "一致性哈希对象使用cookie时，cookie对象不能为空", 400),
	InvalidConsistentHashHttpCookieName("InvalidConsistentHashHttpCookieName", "Http cookie name is invalid",
	                                    "一致性哈希对象使用cookie时，cookie名称不能为空", 400),
	InvalidConsistentHashHttpCookieTtl("InvalidConsistentHashHttpCookieName", "Http cookie ttl is invalid",
	                                   "一致性哈希对象使用cookie时，cookie ttl不能小于0", 400),
	InvalidConsistentHashHttpHeaderName("InvalidConsistentHashHttpHeaderName", "Http header name is invalid",
	                                    "一致性哈希对象使用HttpHeaderName时，HttpHeaderName不能为空", 400),
	InvalidConsistentHashSourceIP("InvalidConsistentHashSourceIP", "Source ip is invalid", "一致性哈希对象使用源IP时，源IP不能为空",
	                              400),

	InvalidHttp1MaxPendingRequests("InvalidHttp1MaxPendingRequests", "http1MaxPendingRequests is invalid",
	                               "http1MaxPendingRequests不能小于0", 400),
	...
	InvalidMaxRequestsPerConnection("InvalidMaxRequestsPerConnection", "MaxRequestsPerConnection is invalid",
	                                "maxRequestsPerConnection不能小于0", 400),
	...
	InvalidSubsetStaticAddr("InvalidSubsetStaticAddr", "Subset's address is invalid", "版本中的地址不合法，不能为空且需要包含在服务发布地址列表中",
	                        400),
	DuplicatedSubsetStaticAddr("DuplicatedSubsetStaticAddr", "Subset's addresses are duplicated", "版本中的地址不合法，存在重复的地址",
	                           400),
	...
	SameNamePluginTemplateExist("SameNamePluginTemplateExist",
	                            "The plugin template with the same name already exists and cannot be created.",
	                            "同名插件模板已存在，不允许重复创建!", 400),
	NoSuchPluginTemplate("NoSuchPluginTemplate", "No such plugin template", "指定的插件模板不存在", 400),

	CannotUpdatePlugin("CannotUpdatePlugin",
	                   "Because the template is associated, you are not allowed to update the configuration",
	                   "关联了模板，不允许直接更新插件配置", 400),

	FailedToPublishServiceOrRoute("FailedToPublishServiceOrRoute", "Failed to automatically publish service or route",
	                              "自动发布服务或路由失败", 400),
	FailedToOfflineServiceOrRoute("FailedToOfflineServiceOrRoute", "Failed to automatically offline service or route",
	                              "自动下线服务或路由失败", 400);
