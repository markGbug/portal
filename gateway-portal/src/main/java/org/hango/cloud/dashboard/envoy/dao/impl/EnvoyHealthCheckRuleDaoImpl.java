...
public class EnvoyHealthCheckRuleDaoImpl extends BaseDao implements EnvoyHealthCheckRuleDao {
	...
	public long add(EnvoyHealthCheckRuleInfo envoyHealthCheckRuleInfo) {
		...
		String sql =
			"insert into apigw_envoy_health_check_rule (create_time, update_time, service_id, gw_id, active_switch, "
			+ "path, timeout, expected_statuses, healthy_interval, healthy_threshold, "
			+ "unhealthy_interval, unhealthy_threshold, passive_switch, consecutive_errors, base_ejection_time, "
			+ "max_ejection_percent, min_health_percent) values(:createTime, :updateTime, :serviceId, :gwId, "
			+ ":activeSwitch,"
			+ ":path, :timeout, :expectedStatuses, :healthyInterval, :healthyThreshold, :unhealthyInterval, "
			+ ":unhealthyThreshold, :passiveSwitch, :consecutiveErrors, :baseEjectionTime, :maxEjectionPercent, "
			+ ":minHealthPercent)";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyHealthCheckRuleInfo);
		namedParameterJdbcTemplate.update(sql, ps, keyHolder);
		logger.info("add envoyHealthCheckRule: {}",
		            ReflectionToStringBuilder.toString(envoyHealthCheckRuleInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public long update(EnvoyHealthCheckRuleInfo envoyHealthCheckRuleInfo) {
		String sql =
			"update apigw_envoy_health_check_rule set update_time=:updateTime, service_id=:serviceId, gw_id=:gwId, "
			+ "active_switch=:activeSwitch, path=:path, "
			+ "timeout=:timeout, expected_statuses=:expectedStatuses, healthy_interval=:healthyInterval, "
			+ "healthy_threshold=:healthyThreshold, unhealthy_interval=:unhealthyInterval,"
			+ "unhealthy_threshold=:unhealthyThreshold, passive_switch=:passiveSwitch, "
			+ "consecutive_errors=:consecutiveErrors, base_ejection_time=:baseEjectionTime, "
			+ "max_ejection_percent=:maxEjectionPercent, min_health_percent=:minHealthPercent where id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyHealthCheckRuleInfo);
		logger.info("update envoyHealthCheckRule: {}",
		            ReflectionToStringBuilder.toString(envoyHealthCheckRuleInfo, ToStringStyle.SIMPLE_STYLE));
	...
	public int delete(EnvoyHealthCheckRuleInfo envoyHealthCheckRuleInfo) {
		...
		SqlParameterSource ps = new BeanPropertySqlParameterSource(envoyHealthCheckRuleInfo);
		logger.info("delete envoyHealthCheckRule: {}",
		            ReflectionToStringBuilder.toString(envoyHealthCheckRuleInfo, ToStringStyle.SIMPLE_STYLE));
		return namedParameterJdbcTemplate.update(sql, ps);
	}
