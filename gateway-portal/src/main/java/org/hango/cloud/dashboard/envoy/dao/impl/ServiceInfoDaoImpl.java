...
public class ServiceInfoDaoImpl extends BaseDao implements ServiceInfoDao {
	...
	public long add(ServiceInfo serviceInfo) {
		...
		String sql =
			"insert into apigw_gportal_service (create_date, modify_date, display_name, service_name, contacts, "
			+ "description,service_type,project_id,sync_status,ext_service_id) "
			+ " values (:createDate, :modifyDate, :displayName, :serviceName, :contacts, :description,:serviceType, "
			+ ":projectId,:syncStatus,:extServiceId)";
	...
	public long update(ServiceInfo serviceInfo) {
		String sql = "update apigw_gportal_service set modify_date = :modifyDate, display_name = :displayName, "
		             + "service_name=:serviceName, contacts=:contacts, "
		             + "description=:description, status=:status, health_interface_path=:healthInterfacePath,"
		             + "sync_status=:syncStatus,ext_service_id=:extServiceId where id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(serviceInfo);
	...
	public List<ServiceInfo> getServiceByProjectId(long projectId) {
		...
		return namedParameterJdbcTemplate.query(sql, new MapSqlParameterSource("projectId", projectId),
		                                        new ServiceInfoRowMapper());
	}
	...
	public List<ServiceInfo> getServiceByServiceName(String serviceName) {
		...
		return namedParameterJdbcTemplate.query(sql, new MapSqlParameterSource("serviceName", serviceName),
		                                        new ServiceInfoRowMapper());
	}
	...
	public List<ServiceInfo> getServiceByProjectIdLimit(String pattern, long offset, long limit, long projectId) {
		...
		if (StringUtils.isNotBlank(pattern)) {
			sql = "select * from apigw_gportal_service where project_id=:projectId and (service_name like :pattern or "
			      + "display_name like :pattern) order by id desc limit :limit offset :offset";
			params.put("pattern", "%" + pattern + "%");
		} else {
			sql = "select * from apigw_gportal_service where project_id=:projectId order by id desc limit :limit "
			      + "offset :offset";
		}
	...
	public long getServiceCountByProjectId(String pattern, long projectId) {
		String sql =
			"select count(*) from apigw_gportal_service where project_id=:projectId and (service_name like :pattern "
	...
	public List<Long> getServiceIdListByDisplayNameFuzzy(String serviceName, long projectId) {
		String sql
			= "select id from apigw_gportal_service where project_id=:projectId and display_name like :serviceName";
