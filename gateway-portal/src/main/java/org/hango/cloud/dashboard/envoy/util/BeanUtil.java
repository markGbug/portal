...
public class BeanUtil {
	...
	public static <T, V> void replaceValueWithBeans(List<T> beans, String property, V value) {
		...
		try {
			...
			Method setMethod = clazz.getMethod("set" + property.substring(0, 1).toUpperCase() + property.substring(1),
			                                   parameterClass);
			for (T t : beans) {
				setMethod.invoke(t, value);
			}
		} catch (NoSuchFieldException e) {
			logger.error("replaceValueWithBeans error, property = {} , value = {}, error = {}", property, value,
			             e.getMessage());
