...
public class ApiDtoGenerateService {
	...
	public List<ApiStatusCode> getStatusCodeFromJson(List<ApiStatusCodeBasicDto> apiStatusCodeDtos, long apiId) {
		if (CollectionUtils.isEmpty(apiStatusCodeDtos)) {
	...
	public ApiInfo getApiInfoFromJson(ApiInfoBasicDto apiInfoDto, ApiExampleDto apiExampleDto, long serviceId,
	                                  long projectId) {
	...
	public List<ApiBody> getApiBodyFromJson(List<ApiBodyBasicDto> apiBodyDtos, String type, long apiId,
	                                        long serviceId) {
		List<ApiBody> apiBodies = new ArrayList<>();
		if (CollectionUtils.isEmpty(apiBodyDtos)) {
			return apiBodies;
		}
		...
		apiBodyDtos.forEach(apiBodyDto -> {
			...
			//具体数组类型的dataTypeId获取
			apiBody.setArrayDataTypeId(
				apiParamTypeService.generateExactByService(apiBody.getArrayDataTypeName(), serviceId));
	...
	public List<ApiHeader> getApiHeaderFromJson(List<ApiHeaderBasicDto> apiHeadDtos, String type, long apiId) {
		if (CollectionUtils.isEmpty(apiHeadDtos)) {
			return new ArrayList<>();
		}
