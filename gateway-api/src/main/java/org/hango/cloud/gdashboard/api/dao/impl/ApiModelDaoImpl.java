...
public class ApiModelDaoImpl extends BaseDao implements ApiModelDao {
	...
	public long add(ApiModel apiModel) {
		...
		String sql = "INSERT INTO " + api_model_table
		             + "(create_date, modify_date,model_name, description, type, format, service_id, swagger_sync,"
		             + "project_id) "
		             + "VALUES (:createDate, :modifyDate,:modelName, :description, :type, :format, :serviceId, "
		             + ":swaggerSync,:projectId)";
	...
	public int update(ApiModel apiModel) {
		String sql = "UPDATE " + api_model_table
		             + " SET model_name=:modelName, description=:description, modify_date=:modifyDate, type=:type "
		             + ", format=:format, service_id = :serviceId, swagger_sync = :swaggerSync, project_id = "
		             + ":projectId where id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(apiModel);
	...
	public List<ApiModel> findApiModelByProjectLimit(long projectId, long offset, long limit, String pattern) {
		...
		if (StringUtils.isNotBlank(pattern)) {
			sql = "select * from " + api_model_table
			      + " where project_id=:projectId and model_name like :pattern order by id desc limit :limit offset "
			      + ":offset";
	...
	public List<ApiModel> findApiModelByServiceIdLimit(long serviceId, long offset, long limit, String pattern) {
		...
		if (StringUtils.isNotBlank(pattern)) {
			sql = "select * from " + api_model_table
			      + " where service_id=:serviceId and model_name like :pattern order by id desc limit :limit offset "
			      + ":offset";
			params.put("pattern", "%" + pattern + "%");
		} else {
			sql = "select * from " + api_model_table
	...
	public long getApiModelCountByProjectPattern(long projectId, String pattern) {
		...
		if (StringUtils.isNotBlank(pattern)) {
			sql = "select count(*) from " + api_model_table
			      + " where project_id=:projectId and model_name like :pattern";
			params.put("pattern", "%" + pattern + "%");
		} else {
	...
	public long getApiModelCountByServicePattern(long serviceId, String pattern) {
		...
		if (StringUtils.isNotBlank(pattern)) {
			sql = "select count(*) from " + api_model_table
			      + " where service_id=:serviceId and model_name like :pattern";
			params.put("pattern", "%" + pattern + "%");
		} else {
