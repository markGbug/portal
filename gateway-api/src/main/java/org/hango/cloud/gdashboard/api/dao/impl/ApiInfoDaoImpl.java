...
public class ApiInfoDaoImpl extends BaseDao implements ApiInfoDao {
	...
	public long add(ApiInfo apiInfo) {
		...
		String sql = "INSERT INTO " + api_table
		             + "(create_date,modify_date, api_name,api_path, api_method, description, type, service_id, regex,"
		             + " document_status_id, request_example_value, response_example_value,alias_name,sync_status,"
		             + "ext_api_id,project_id,swagger_sync) "
		             + "VALUES (:createDate,:modifyDate, :apiName, :apiPath, :apiMethod, :description, :type, "
		             + ":serviceId, :regex, :documentStatusId, :requestExampleValue, :responseExampleValue,:aliasName,"
		             + ":syncStatus,:extApiId,:projectId,:swaggerSync)";
	...
	public int update(ApiInfo apiInfo) {
		String sql = "update " + api_table
		             + " set api_name=:apiName, api_path=:apiPath, api_method=:apiMethod, modify_date=:modifyDate,"
		             + "description=:description, type=:type, status=:status, regex=:regex, "
		             + "document_status_id=:documentStatusId, request_example_value=:requestExampleValue, "
		             + "response_example_value=:responseExampleValue , alias_name=:aliasName,sync_status=:syncStatus,"
		             + "ext_api_id=:extApiId, swagger_sync=:swaggerSync where id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(apiInfo);
	...
	public long addWithId(ApiInfo apiInfo) {
		String sql = "INSERT INTO " + api_table
		             + " (id, create_date, api_name,api_path, api_method, type, service_id, action, version, status, "
		             + "id_cert) "
		             + "VALUES (:id, :createDate, :apiName, :apiPath, :apiMethod, :type, :serviceId, :action, "
		             + ":version, :status, :idCert)";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(apiInfo);
		return namedParameterJdbcTemplate.update(sql, ps);
	}

	@Override
	public List<ApiInfo> findAllApiInfoByServiceLimit(long serviceId, long documentStatusId, String pattern,
		...
		if (documentStatusId != 0) {
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select * from " + api_table
				      + " where service_id=:serviceId and document_status_id=:documentStatusId  and (api_name like "
				      + ":pattern  or api_path like :pattern) order by id desc limit :limit offset :offset";
				params.put("pattern", "%" + pattern + "%");
			} else {
				sql = "select * from " + api_table
				      + " where service_id=:serviceId and document_status_id=:documentStatusId order by id desc limit "
				      + ":limit offset :offset";
			}
			...
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select * from " + api_table
				      + " where service_id=:serviceId  and (api_name like :pattern  or api_path like :pattern) order "
				      + "by id desc limit :limit offset :offset";
				params.put("pattern", "%" + pattern + "%");
			} else {
				sql = "select * from " + api_table
				      + " where service_id=:serviceId  order by id desc limit :limit offset :offset";
			}
		}
	...
	@Override
	public List<ApiInfo> findAllApiInfoByProjectLimit(long projectId, long documentStatusId, String pattern,
	                                                  long offset, long limit) {
		String sql;
		Map<String, Object> params = new HashMap<String, Object>();
		if (documentStatusId != 0) {
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select * from " + api_table
				      + " where project_id=:projectId and document_status_id=:documentStatusId  and (api_name like "
				      + ":pattern  or api_path like :pattern) order by id desc limit :limit offset :offset";
				params.put("pattern", "%" + pattern + "%");
			} else {
				sql = "select * from " + api_table
				      + " where project_id=:projectId and document_status_id=:documentStatusId order by id desc limit "
				      + ":limit offset :offset";
			}
			params.put("documentStatusId", documentStatusId);
		} else {
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select * from " + api_table
				      + " where project_id=:projectId and (api_name like :pattern  or api_path like :pattern) order by"
				      + " id desc limit :limit offset :offset";
				params.put("pattern", "%" + pattern + "%");
			} else {
				sql = "select * from " + api_table
				      + " where project_id=:projectId order by id desc limit :limit offset :offset";
			}
		}
		params.put("offset", offset);
		params.put("limit", limit);
	...
	public long getApiCountByService(long serviceId, long documentStatusId, String pattern) {
		...
		if (documentStatusId != 0) {
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select count(*) from " + api_table
				      + "  where service_id=:serviceId and document_status_id=:documentStatusId  and (api_name like "
				      + ":pattern  or api_path like :pattern)";
				params.put("pattern", "%" + pattern + "%");
			} else {
				sql = "select count(*) from " + api_table
				      + "  where service_id=:serviceId and document_status_id=:documentStatusId";
			}
			params.put("documentStatusId", documentStatusId);
		} else {
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select count(*) from " + api_table
				      + "  where service_id=:serviceId  and (api_name like :pattern  or api_path like :pattern)";
				params.put("pattern", "%" + pattern + "%");
			} else {
	...
	public long getApiCountByProject(long projectId, long documentStatusId, String pattern) {
		...
		if (documentStatusId != 0) {
			if (StringUtils.isNotBlank(pattern)) {
				...
				sql = "select count(*) from " + api_table
				      + "  where project_id=:projectId and document_status_id=:documentStatusId";
			}
			params.put("documentStatusId", documentStatusId);
		} else {
			if (StringUtils.isNotBlank(pattern)) {
				sql = "select count(*) from " + api_table
				      + "  where project_id=:projectId  and (api_name like :pattern  or api_path like :pattern)";
				params.put("pattern", "%" + pattern + "%");
			} else {
