...
public class ApiInfoImpl implements IApiInfoService {
	...
	public ApiErrorCode checkParamApiBasicDto(ApiInfoBasicDto apiInfoBasicDto) {
		...
		//同一服务不能存在相同的服务标识
		if (StringUtils.isNotBlank(apiInfoBasicDto.getAliasName()) && !isExistAliasName(
			String.valueOf(apiInfoBasicDto.getServiceId()), apiInfoBasicDto.getAliasName(), apiInfoBasicDto.getId())) {
			logger.info("同一服务下不能存在相同的API标识,aliasName:{}", apiInfoBasicDto.getAliasName());
			return CommonApiErrorCode.AlreadyExistAliasName;
		}

		//判断API是否已经存在
		if (isRestfulApiExists(apiInfoBasicDto.getApiPath(), apiInfoBasicDto.getApiMethod(),
		                       apiInfoBasicDto.getServiceId(), apiInfoBasicDto.getId())) {
	...
	@Override
	public long addApiInfos(final ApiInfoBasicDto apiInfoBasicDto, final String type) {
	...
	@Override
	public List<ApiInfo> findAllApiByProjectLimit(long projectId, long serviceId, long apiDocumentId, String pattern,
