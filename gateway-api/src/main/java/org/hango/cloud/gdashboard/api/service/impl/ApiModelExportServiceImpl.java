...
public class ApiModelExportServiceImpl implements IApiModelExportService {
	...
	public void addApiModel(List<CreateApiModelDto> createApiModelDtos, long serviceId) {
		...
		//去除重复的model name
		List<CreateApiModelDto> collect = createApiModelDtos.stream().filter(
			distinctByKey(CreateApiModelDto::getModelName)).collect(Collectors.toList());
	...
	public void addJsonModel(List<CreateApiModelDto> createApiModelDtos) {
		...
		createApiModelDtos.forEach(createApiModelDto -> {
			long modelId = 0;
			ApiModel apiModel = apiModelService.getApiModelByServiceIdAndModelName(createApiModelDto.getServiceId(),
			...
			//构造paramType ID
			long modelTypeId = 0;
			ApiParamType apiParamType = apiParamTypeService.listApiParamTypeByModelId(createApiModelDto.getModelName(),
			                                                                          modelId);
			...
			createApiModelDtos.forEach(createApiModelDto -> {
				...
				if (!CollectionUtils.isEmpty(params)) {
					params.forEach(apiParamDto -> {
						apiParamDto.setParamTypeId(apiParamTypeService
							                           .generateExactByService(apiParamDto.getParamTypeName(),
							                                                   createApiModelDto.getServiceId()));
						apiParamDto.setArrayDataTypeId(apiParamTypeService
							                               .generateExactByService(apiParamDto.getArrayDataTypeName(),
							                                                       createApiModelDto.getServiceId()));
					});
				}
				apiModelService.addApiModelParam(createApiModelDto, modelIdMap.get(createApiModelDto.getModelName()),
				                                 apiModelType);
			});
		}
	}
