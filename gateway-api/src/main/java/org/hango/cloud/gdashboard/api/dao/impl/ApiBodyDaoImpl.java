...
public class ApiBodyDaoImpl extends BaseDao implements ApiBodyDao {
	...
	public long add(ApiBody apiBody) {
		...
		String sql = "INSERT INTO " + body_param_table
		             + "(create_date, modify_date, api_id, param_name, param_type_id, array_data_type_id, required, "
		             + "def_value, description, type,association_type) "
		             + "VALUES (:createDate, :modifyDate, :apiId, :paramName, :paramTypeId, :arrayDataTypeId, "
		             + ":required, :defValue, :description, :type,:associationType);";
	...
	public int update(ApiBody apiBody) {
		String sql = "UPDATE " + body_param_table
		             + " SET modify_date=:modifyDate, param_name=:paramName, required=:required, "
		             + "param_type_id=:paramTypeId, array_data_type_id=:arrayDataTypeId, required=:required, "
		             + "def_value=:defValue, description=:description  WHERE id=:id";
		SqlParameterSource ps = new BeanPropertySqlParameterSource(apiBody);
	...
	public List<ApiBody> getBodyParam(String paramName, String type, long apiId) {
		...
		mapSqlParameterSource.addValue("apiId", apiId);
		String sql = "select * from " + body_param_table
		             + " where param_name = :paramName and type=:type and api_id = :apiId";
		return namedParameterJdbcTemplate.query(sql, mapSqlParameterSource, new ApiBodyRowMapper());
	}
