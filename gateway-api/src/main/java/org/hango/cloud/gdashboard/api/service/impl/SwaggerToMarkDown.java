...
public class SwaggerToMarkDown implements ISwaggerToMarkDown {
	...
	@Override
	public Map<String, String> swaggerToMd(String swagger, long apiId, List<ApiStatusCode> apiStatusCodeList,
	                                       long id) {
		...
		if (!(parametersList == null || parametersList.size() == 0)) {
			...
			for (JSONObject parameter : parametersList) {
				...
				if (parameterMap.get("type") != null) {
					parametersString += "| " + parameterMap.get("name") + " | " + convertReturnToBr(
						(String) parameterMap.get("description")) + " | " + detailedType + " | " + requiredFlag
					                    + " | |\n";
					flag = true;
				} else {
			...
			if (apiInfo.getType().equals(Const.API_ACTION_TYPE)) {
				...
				for (ApiStatusCode apiStatusCode : apiStatusCodeList) {
					...
					responsesString += "| " + apiStatusCode.getErrorCode() + " | " + apiStatusCode.getMessage() + " | "
					                   + apiStatusCode.getStatusCode() + " | " + convertReturnToBr(
						(String) apiStatusCode.getDescription()) + " |\n";
				}
			} else {
				...
				for (ApiStatusCode apiStatusCode : apiStatusCodeList) {
					...
					responsesString += "| " + apiStatusCode.getStatusCode() + " | " + convertReturnToBr(
						(String) apiStatusCode.getDescription()) + " |\n";
				}
			}
		} else {
	...
	@Override
	public void generateModelString(String subParameterName, Map<String, Object> modelMap,
	                                Map<String, Object> definitionsMap, Stack<String> subParameterStringList,
	                                long id) {
		...
		if (modelMap != null && modelMap.size() != 0) {
			...
			Map<String, JSONObject> propertiesMap = modelMap.get("properties") != null ? (Map) modelMap.get(
				"properties") : null;

			if (!CollectionUtils.isEmpty(propertiesMap)) {
				subString += "| 参数名称 | 说明 | 参数类型 | 是否必填 | 备注 |\n";
				subString += "| --- | --- | --- | --- | --- |\n";
				subString = generateSubString(requiredList, propertiesMap, subString, definitionsMap,
				                              subParameterStringList, id);
			} else if (modelMap.get("type") instanceof String && "array".equalsIgnoreCase(
				(String) modelMap.get("type"))) {
	...
	@Override
	public String generateArrayString(Entry<String, JSONObject> arrayMap, String subString, String requiredFlag,
	                                  Map<String, Object> definitionsMap, Stack<String> subParameterStringList,
	                                  long id) {
		...
		if (!(((JSONObject) arrayMap.getValue().get("items")).get("type") == null)) {
			...
			if (Const.BLANK_ARRAY_CONST.equalsIgnoreCase(arrayMap.getKey())) {
				subString += "| | " + convertReturnToBr((String) arrayMap.getValue().get("description"))
				             + " | array<span>&lt;</span>" + detailedType + "<span>&gt;</span> | " + requiredFlag
				             + " | |\n";
			} else {
				subString += "| " + arrayMap.getKey() + " | " + convertReturnToBr(
					(String) arrayMap.getValue().get("description")) + " | array<span>&lt;</span>" + detailedType
				             + "<span>&gt;</span> | " + requiredFlag + " | |\n";
			}
		} else {
			// Array参数类型为引用类型
			id++;
			...
			if (Const.BLANK_ARRAY_CONST.equalsIgnoreCase(subParameterName)) {
				subString += "| | " + convertReturnToBr((String) arrayMap.getValue().get("description"))
				             + " | array[<span>&lt;</span>" + modelName + "<span>&gt;</span>](#" + id + ") | "
				             + requiredFlag + " | 详细信息见下表 |\n";
			} else {
				subString += "| [" + subParameterName + "](#" + id + ") | " + convertReturnToBr(
					(String) arrayMap.getValue().get("description")) + " | array[<span>&lt;</span>" + modelName
				             + "<span>&gt;</span>](#" + id + ") | " + requiredFlag + " | 详细信息见下表 |\n";
			}
	...
	private String generateSubString(List<String> requiredList, Map<String, JSONObject> propertiesMap,
	                                 String subString,
	                                 Map<String, Object> definitionsMap, Stack<String> subParameterStringList,
	                                 long id) {
		for (Entry<String, JSONObject> entry : propertiesMap.entrySet()) {
			...
			if (!(entry.getValue().get("type") == null)) {
				...
				if (!"array".equals(entry.getValue().get("type"))) {
					// 参数不是Array类型
					subString += "| " + entry.getKey() + " | " + convertReturnToBr(
						(String) entry.getValue().get("description")) + " | " + detailedType + " | " + requiredFlag
					             + " | |\n";
				} else {
					// 参数是Array类型
					subString = generateArrayString(entry, subString, requiredFlag, definitionsMap,
					                                subParameterStringList, id);
				}
			} else {
				// 参数是Model类型
				...
				subString += "| [" + subparameterName + "](#" + id + ") | " + convertReturnToBr(
					(String) entry.getValue().get("description")) + " | [" + subModelName + "](#" + id + ")| "
				             + requiredFlag + " | 详细信息见下表 |\n";
				Map<String, Object> subModelMap = (Map) definitionsMap.get(subModelName);
				generateModelString("**" + subModelName + "**", subModelMap, definitionsMap, subParameterStringList,
				                    id);
			}
		}
		return subString;
