...
public class ApiModelServiceImpl implements IApiModelService {
	...
	public String getApiModelRefer(long modelId) {
		...
		if (apiBodyList1.size() == 0 && apiBodyList2.size() == 0 && apiModelList1.size() == 0
		    && apiModelList2.size() == 0) {
			return message;
		} else {
			//可优化下提示
		...
		for (ApiModelParam apiModelParam : apiModelParamList) {
			...
			if (apiModelParam.getArrayDataTypeId() != 0) {
				apiParamDto.setArrayDataTypeName(
					apiParamTypeDao.get(apiModelParam.getArrayDataTypeId()).getParamType());
			} else {
				apiParamDto.setArrayDataTypeName("");
			}
	...
	@Override
	public List<ApiModel> findAllApiModelByProjectLimit(long serviceId, long project, long offset, long limit,
	                                                    String pattern) {
		if (serviceId == 0) {
	...
	public ApiErrorCode checkAddApiModelParam(CreateApiModelDto createApiModelDto) {
		...
		for (ApiParamDto apiParamDto : createApiModelDto.getParams()) {
			if (!apiParamTypeService.isApiParamTypeExists(apiParamDto.getParamTypeId())) {
				...
				return CommonApiErrorCode.InvalidParameter(String.valueOf(apiParamDto.getParamTypeId()), 
				                                           "ParamTypeId");
			}
		}

		List<ApiParamDto> apiParamDtos = createApiModelDto.getParams();
		if (!CollectionUtils.isEmpty(apiParamDtos) && apiParamDtos.size() != new HashSet<ApiParamDto>(apiParamDtos)
			                                                                     .size()) {
		...
		//        //ModelName中的参数命名不能一样
		//        if (apiParamTypeService.listParamTypeInHeaderAndBodyButNotModel().contains(createApiModelDto
		//        .getModelName())) {
