...
public class ApiExportServiceImpl implements IApiExportService {
	...
	public List<ApiExportDto> getApiInfo(long serviceId, String serviceType) {
		...
		for (ApiInfo apiInfo : apiInfos) {
			...
			List<ApiHeaderBasicDto> apiRequestHeaders = apiDtoGenerateService.getApiHeader(apiInfo.getId(),
			                                                                               Const.REQUEST_PARAM_TYPE);
			List<ApiHeaderBasicDto> apiResponseHeaders = apiDtoGenerateService.getApiHeader(apiInfo.getId(),
			...
			List<ApiBodyBasicDto> apiQueryString = apiDtoGenerateService.getApiBody(apiInfo.getId(),
			...
			List<ApiBodyBasicDto> apiRequestBody = apiDtoGenerateService.getApiBody(apiInfo.getId(),
			                                                                        Const.REQUEST_PARAM_TYPE);
			List<ApiBodyBasicDto> apiResponseBody = apiDtoGenerateService.getApiBody(apiInfo.getId(),
			                                                                         Const.RESPONSE_PARAM_TYPE);
	...
	public void addApiInfos(List<ApiExportDto> apiExportDtos, long serviceId, String serviceType, long projectId) {
		if (CollectionUtils.isEmpty(apiExportDtos)) {
			return;
		}
		apiExportDtos.forEach(apiExportDto -> {
			if (apiExportDto.getApiInfoBasicDto() == null) {
				return;
			}

			ApiInfo apiInfo = apiInfoService.getApiInfoByApiPathAndService(
				apiExportDto.getApiInfoBasicDto().getApiPath(), apiExportDto.getApiInfoBasicDto().getApiMethod(),
				serviceId);
			long apiId = 0;
			//修改API信息
			if (apiInfo != null) {
				...
				apiInfo = apiDtoGenerateService.getApiInfoFromJson(apiExportDto.getApiInfoBasicDto(),
				                                                   apiExportDto.getApiExampleDto(), serviceId,
				                                                   projectId);
				//添加API基本信息
			...
			if (apiExportDto.getRequestHeader() != null) {
				requestHeader = apiDtoGenerateService.getApiHeaderFromJson(apiExportDto.getRequestHeader(),
				                                                           Const.REQUEST_PARAM_TYPE, apiId);
			}
			if (apiExportDto.getResponseHeader() != null) {
				responseHeader = apiDtoGenerateService.getApiHeaderFromJson(apiExportDto.getRequestHeader(),
				                                                            Const.RESPONSE_PARAM_TYPE, apiId);
			}
			//添加api header
			...
			List<ApiBody> queryString = apiDtoGenerateService.getApiBodyFromJson(apiExportDto.getQueryString(),
			                                                                     Const.QUERYSTRING_PARAM_TYPE, apiId,
			                                                                     serviceId);

			List<ApiBody> requestBody = apiDtoGenerateService.getApiBodyFromJson(apiExportDto.getRequestBody(),
			                                                                     Const.REQUEST_PARAM_TYPE, apiId,
			                                                                     serviceId);
			List<ApiBody> responseBody = apiDtoGenerateService.getApiBodyFromJson(apiExportDto.getResponseBody(),
			                                                                      Const.RESPONSE_PARAM_TYPE, apiId,
			                                                                      serviceId);
			...
			List<ApiStatusCode> apiStatusCodes = apiDtoGenerateService.getStatusCodeFromJson(
				apiExportDto.getStatusCode(), apiId);
