		...
		if (!CollectionUtils.isEmpty(apiBodyBasicDtos)) {
			...
			apiBodies.forEach(apiBody -> {
				...
				if (apiBody.getArrayDataTypeId() != 0) {
					apiBody.setArrayDataTypeName(
						apiParamTypeService.listApiParamType(apiBody.getArrayDataTypeId()).getParamType());
				}
	...
	public ApiErrorCode checkApiBodyBasicInfo(ApiBodysDto apiBodysDto) {
		...
		if (!CollectionUtils.isEmpty(apiBodysDto.getApiBodyBasicDtoList())) {
			for (ApiBodyBasicDto apiBodyBasicDto : apiBodysDto.getApiBodyBasicDtoList()) {
				...
				if (apiBodyBasicDto.getArrayDataTypeId() != 0 && apiParamTypeService.listApiParamType(
					apiBodyBasicDto.getArrayDataTypeId()) == null) {
					return CommonApiErrorCode.NoSuchArrayDataType;
				}
				String paramType = apiParamTypeService.listApiParamType(apiBodyBasicDto.getParamTypeId())
				                                      .getParamType();
				if (Const.BLANK_ARRAY_CONST.equals(apiBodyBasicDto.getParamName()) && !"Array".equalsIgnoreCase(
					paramType)) {
			...
			List<String> apiBodyParamName = apiBodysDto.getApiBodyBasicDtoList().stream().map(
				ApiBodyBasicDto::getParamName).collect(Collectors.toList());
